...
function(x, y, Z, offset, psi2, wei, Bx, By,
                                  nbx, nby, RTBx, RTBy, lambdas, Px,
                                  Py, a.init,
                                  MON, TOL, MAX.IT){
    # Input:
        # x: abcissae of data
        ...
        # TOL: vector of the two relative convergence tolerances for the smoothing parameters
    ...
    # diagnostics
    Z1 <- Z
    Z1[Z==0] <- 10^(-4) # replace zeros in response
    # deviance
    dev <- 2*(sum(Z*wei * log(Z1/mu), na.rm = TRUE))
    # effective dimension
    df <- sum(h)
    ...
    # output
    llist <- list(a=a, h=h, df=df, aic=aic, bic=bic, dev=dev, tol=tol)
    llist
}
